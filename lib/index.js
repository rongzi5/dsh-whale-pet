...
function createTaskHandler(subagents, agents, workspaceRoot) {
  return async (req, res) => {
    const url = new URL(req.url ?? "/", "http://localhost");
    ...
    if (body === null || typeof body !== "object") {
      sendJson3(res, 400, { error: "request body must be JSON: { prompt: string }" });
      return;
    }
    const record = body;
...
// src/index.ts
var inject = ["webServer", "llm", "credentials", "sessions", "jobs", "subagents", "agents"];
